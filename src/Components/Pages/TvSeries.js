...
const TvSeries = () => {
  ...
  const fetchTvSeries = async () => {
    try {
      let url = "";
      if (searchText) {
        url = `https://api.themoviedb.org/3/search/tv?api_key=${process.env.REACT_APP_API_KEY}&page=${page}&query=${searchText}`;
      } else {
        url = `https://api.themoviedb.org/3/discover/tv?api_key=${process.env.REACT_APP_API_KEY}&page=${page}&with_genres=${genreforURL}`;
      }
  ...
  return (
    <div style={{ paddingTop: "2em", backgroundColor: "black" }}>
